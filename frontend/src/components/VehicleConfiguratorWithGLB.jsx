...
// GLB Car Model Component
function GLBCarModel({ color, modelPath = '/models/car.glb' }) {
  ...
  useEffect(() => {
    ...
    scene.traverse((child) => {
      if (child.isMesh) {
        ...
        // Adjust this logic based on your specific model's material names
        if (child.material) {
          const materialName = child.material.name?.toLowerCase() || '';
          
          // Apply color to body parts (adjust names based on your model)
...
// Fallback: Basic Geometric Car (if GLB fails to load)
function BasicCarModel({ color }) {
  const meshRef = useRef();
  const autoRotate = useConfiguratorStore((state) => state.autoRotate);
...
// Smart Car Model Loader (tries GLB first, falls back to basic)
function CarModel({ color }) {
  const USE_GLB_MODEL = false; // Set to true when you have a GLB file in public/models/car.glb
  
  if (USE_GLB_MODEL) {
    return <GLBCarModel color={color} modelPath="/models/car.glb" />;
  }
  
  return <BasicCarModel color={color} />;
}
...
// Preload GLB model (uncomment when you have the file)
